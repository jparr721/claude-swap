...
class ClaudeAccountSwitcher:
    ...
    def _reject_live_api_key_capture(self, creds: str) -> None:
        ...
        if looks_like_api_key(creds):
            raise ValidationError(
                ...
                "'cswap claude add-token sk-ant-api...' instead of "
                "'cswap claude add'."
            )
    ...
    def first_run_setup(self) -> None:
        ...
        if response.lower() == "n":
            print(dimmed("Setup cancelled. You can run 'cswap claude add' later."))
            return
    ...
    def switch(
        ...
        better, it stays put (run a plain ``cswap claude switch`` to rotate anyway).
        ...
        # (e.g. right after cswap claude import). Activate the recorded
        ...
        if not self._account_exists(current_email, current_org_uuid):
            ...
            if json_output:
                ...
                return self._switch_noop(
                    ...
                    from_ref=ref,
                    to_ref=ref,
                    message="Active account is not managed; run cswap claude add",
                )
            ...
            if not self._account_is_switchable(candidate):
                if json_output:
                    ...
                    print(
                        ...
                        f"(no stored credentials/config, re-add with "
                        f"cswap claude add --slot {candidate})"
                    )
                continue
        ...
        if next_account is None:
            ...
            print(dimmed(
                ...
                "Re-add a skipped slot with: cswap claude add --slot <number>"
            ))
            return None
                ...
                if cur_slot == target_account:
                    ...
                    if not json_output:
                        ...
                        print(dimmed(
                            ...
                            f"cswap claude switch --to {target_account} --force"
                        ))
                        return None
                    return self._switch_noop(
                        strategy="direct",
        ...
        if pre_email:
            ...
            if pids:
                msg = (
                    ...
                    f"'cswap claude run {target_account}'."
                )
                if emit_output:
                    warning(msg)
                else:
            ...
            if force_activate or current_identity is None or current_account is None:
                ...
                if not target_creds:
                    raise SwitchError(
                        ...
                        f"Re-add with: cswap claude add --slot {target_account}"
                    )
    ...
    def purge(self) -> None:
        ...
        print(
            dimmed(
                "Note: Your current Claude Code and Codex logins are preserved."
            )
        )
        print()
