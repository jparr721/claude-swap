...
class OpencodeOpenAIBackend:
    ...
    def metadata_from_text(self, text: str) -> AuthMetadata:
        try:
            ...
            raise ConfigError(f"opencode auth file is not valid JSON: {exc}") from exc
        if not isinstance(data, dict):
            raise ConfigError("opencode auth file must contain a JSON object")
        openai = data.get("openai")
        if isinstance(openai, dict):
            openai_data = openai
        else:
            openai_data = {}
        access_token = _safe_str(openai_data.get("access"))
        refresh_token = _safe_str(openai_data.get("refresh"))
        if not access_token and not refresh_token:
            raise ConfigError("opencode auth file does not contain OpenAI OAuth credentials")
        return AuthMetadata(
            account_id=_safe_str(openai_data.get("accountId")),
            auth_mode=_safe_str(openai_data.get("type")),
            fingerprint=_fingerprint(text),
