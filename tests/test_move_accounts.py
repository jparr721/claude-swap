...
class TestMoveAccount:
    ...
    def test_move_strict_clear_fails_closed_on_locked_keychain(
        ...
        with monkeypatch.context() as patch:
            patch.setattr(macos_keychain, "get_password", locked)
            patch.setattr(macos_keychain, "delete_password", locked)
    ...
    def test_move_metadata_failure_leaves_account_intact(
        ...
        with monkeypatch.context() as patch:
            patch.setattr(ClaudeAccountSwitcher, "_write_json", failing_write_json)
